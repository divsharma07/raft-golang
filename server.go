...
func (s *Server) Serve() {
	...
	log.Print("[%+v] listening at %s", s.serverId, s.listener.Addr())
	s.mu.Unlock()
...
func (s *Server) Call(id int, serviceMethod string, args interface{}, reply interface{}) error {
	...
	if peer == nil {
		return fmt.Errorf("call failed because client %d is closed", id)
	} else {
		return peer.Call(serviceMethod, args, reply)
	}
}

// A proxy that helps add request dropping and/or delay in requests
